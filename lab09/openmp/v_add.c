...
// Edit this function (Method 1) 下面两个函数是我写的
void v_add_optimized_adjacent(double* x, double* y, double* z) {
     #pragma omp parallel
	{
        int num= omp_get_num_threads();//有多少条线
        int index= omp_get_thread_num();//thread ID
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
          #pragma omp parallel
	{
        int num= omp_get_num_threads();//有多少条线
        int index= omp_get_thread_num();//thread ID
        int chunk= ARRAY_SIZE/num;
        int mod= ARRAY_SIZE%num;//取余
        if（index<mode）{//前几个每一个chunk要多加一个，每条thread要操作chunk+1次
		for(int i=index*(chunk+1); i<(index+1)*(chunk+1); i++)
			z[i] = x[i] + y[i];
        }
        else{//后面的chunk，每个thread操作chunk次
            for(int i=mod*(chunk+1)+(index-mod)*chunk;i<mod*(chunk+1)+(index-mode+1)*chunk;i++)
                z[i] = x[i] + y[i];
        }
	}
}
